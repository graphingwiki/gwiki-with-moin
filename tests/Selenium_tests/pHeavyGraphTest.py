# -*- coding: utf-8 -*-
'''This is a Python 2.7 script for testing Collab wiki graph generation function graphingwiki. Modules needed for usage: Selenium webdriver and geckodriver. The script creates a test page to the wiki with several different graphs. If the test is succesful the script outputs the arithmetic mean of the time it took to generate the graphs five times. The time is measured from when the page is saved to when the graphs are first displayed. Change variables site, password and user from test_config.ini to match your setup. 
...
6. The test can now be executed by command: python pHeavyGraphTest.py
...
class HeavyGraphTestCase(unittest.TestCase):
	...
	'''Generates a test page five times with several graphs and then deletes it. Outputs the mean of the time it took to generate the graphs if the test did not generate any errors.'''

	n=int(config.get('pHeavyGraphTest_settings','number_of_tests'))
	...
	print "Average time for generating the graphs: ", avg_time 
