...
class heavyGraphTestCase(unittest.TestCase):
	...
	for i in range(1,n+1):
		...
		new_site = site + "/textTestPage" + str(i)
		driver.get(new_site)
		driver.find_element_by_link_text("Edit").click()
		driver.find_element_by_id("editor-textarea").click()
		driver.find_element_by_id("editor-textarea").clear()
		text = ("Creating testpage %s, #%s" %(i, r))
		driver.find_element_by_id("editor-textarea").send_keys(text)
		t0 = timer()
		...
		driver.get(new_site + '?action=DeletePage')
        	driver.find_element_by_name("delete").click()
	avg_time= sum(total_times) / len(total_times)
	print "Average time for generating a page: ", avg_time 
